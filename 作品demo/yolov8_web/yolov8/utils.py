...
class_names = ['good', 'broke', 'circle', 'loss', 'uncovered']

# Create a list of colors for each class where each color is a tuple of 3 integer values
rng = np.random.default_rng(3)
colors = rng.uniform(0, 255, size=(len(class_names), 3))  # class中的每一个类别生成一个颜色以进行标注
...
def xywh2xyxy(x):  # 边框坐标转换至左上右下点坐标
    # Convert bounding box (x, y, w, h) to bounding box (x1, y1, x2, y2)
    y = np.copy(x)  # x副本 其中0，1为中心点的坐标
    y[..., 0] = x[..., 0] - x[..., 2] / 2  # 左上角的x
    y[..., 1] = x[..., 1] - x[..., 3] / 2  # 左上角的y
    y[..., 2] = x[..., 0] + x[..., 2] / 2
    y[..., 3] = x[..., 1] + x[..., 3] / 2
